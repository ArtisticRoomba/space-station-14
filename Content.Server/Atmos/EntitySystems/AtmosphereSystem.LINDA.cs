...
{
    /// <summary>
    /// Processes a single tile for gas exchange between its neighbors.
    /// </summary>
    /// <param name="ent">The grid entity containing the tile.</param>
    /// <param name="tile">The tile to process.</param>
    /// <param name="fireCount">The current processing cycle count.</param>
        ...
        int fireCount)
    {
        ...
        // Archive the tile's air state if we haven't already this cycle.
        ...
        // Determine how many tiles that can exchange gases with this tile.
        ...
        // Time to go through each tile and exchange gases.
        for (var i = 0; i < Atmospherics.Directions; i++)
        {
            var direction = (AtmosDirection)(1 << i);
            // Air can't flow to this tile.
            if (!tile.AdjacentBits.IsFlagSet(direction))
                continue;
            ...
            if (enemyTile?.Air == null)
                continue;
            // We've already processed this tile this cycle.
    ...
    /// <summary>
    /// Archives the current state of a tile's air and stores it in <see cref="TileAtmosphere.AirArchived"/>.
    /// This allows LINDA to perform gas sharing and temperature sharing based on the state of the air
    /// at the start of the current processing cycle, rather than the state modified by earlier
    /// sharing in the same cycle. Makes LINDA less order-dependent.
    /// </summary>
    /// <param name="tile">The tile to archive.</param>
    /// <param name="fireCount">The current processing cycle count.</param>
    ...
    /// <summary>
    /// Shares gases between two tiles.
    /// </summary>
    /// <param name="tileReceiver">The tile receiving gases.</param>
    /// <param name="tileSharer">The tile sharing gases.</param>
    /// <param name="atmosAdjacentTurfs">The number of adjacent turfs for the sharing tile.</param>
    public float Share(TileAtmosphere tileReceiver, TileAtmosphere tileSharer, int atmosAdjacentTurfs)
    {
        ...
        // Get the current heat capacities of this tile and the sharer tile if temperature delta is significant,
        // as we need it for computing thermal energy transfer later, if necessary.
        ...
        {
            /*
             LINDA exchanges heat energy between tiles using both advective exchange
             (the physical exchanged mass carrying energy) and conductive exchange.
             The following is an attempt (I LOVE NO DOCUMENTATION!!!!!) to explain how the
